...
function Cardcc(props){
    ...
    useEffect(() => {
        if (slug) {
            backfunc.getPostc(slug).then((post) => {
        ...
        if(!user.logged){
            ...
            if(c.length==0){
                ...
                .then((cart)=>{
                    ...
                    const ne='c'+slug+post.price.substring(1);
        ...
        <div className=" flex flex-col justify-center align-middle justify-items-center items-center ">
            ...
            <div className="  shadow-md shadow-slate-500 flex flex-col md:flex-row p-2 bg-white bg-opacity-20 mb-8 lg:w-1/3 md:w-2/3 w-1/2 justify-around rounded-2xl">
                ...
                <p className="flex justify-center justify-enenly">
                    {!added&&<button className="m-2  md:shadow-md md:shadow-slate-500 text-2xl bg-amber-600 rounded-md p-2 " onClick={addCart}>
                        Add to cart
                    </button>}
...
export default connect(mapStateToProps,mapDispatchToProps)(Cardcc)